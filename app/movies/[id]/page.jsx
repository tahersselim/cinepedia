import { getWeekTop10, searchMovies } from "@/app/utils/requests";
...
export default async function MovieDetailsPage({ params }) {
  ...
  const searchmovies = await searchMovies(params.id); // Pass the search query to the API

  // Find the movie details from the search result
  const movieDetail = searchmovies.find((movie) => movie.id === params.id);

  // Find the movie details from the week top 10
  const movieFromTop10 = movies.find((movie) => movie.id === params.id);

  // Handle case when the movie is not found
  if (!movieDetail && !movieFromTop10) {
    return (
  ...
  if (movieFromTop10) {
    const imgUrl = movieFromTop10.primaryImage?.imageUrl || '/default-image.jpg'; // Fallback image
    ...
    const country = movieFromTop10.releaseDate?.country?.text || 'Unknown Country';
    const Rating = movieFromTop10.titleCertificate || {};
    const currentRank = movieFromTop10.chartMeterRanking?.currentRank || 'Not Ranked';
      ...
      <div className="container my-5">
        <div className="row">
          ...
          <div className="col-md-8">
            ...
            <p className="lead">{plot.plotText?.plainText || 'No Plot Available'}</p>
